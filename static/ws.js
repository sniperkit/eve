...
window.onload = function() {
  ...
  function checkNewUser() {
    ...
    if (bots == null) {
      ...
      contentEl.innerHTML = "";
      contentEl.style["background-image"] =
        "url('/static/assets/bgpattern.png')";
  ...
  if (noBotsAvail != true) {
    ...
    if (window["WebSocket"]) {
      ...
      appendChat(
        "Dein Browser unterstützt scheinbar keine WebSockets...",
        "bot"
      );
    }
...
function showPopup(id) {
  ...
  if (id == "profilepopup" && !didLoadHQ) {
    //Load HQ image for profile
        ...
        for (key in list) {
          if (result.hasOwnProperty(list[key])) {
            ...
            item.setAttribute(
              "onclick",
              "setImage(" +
                result[list[key]].ImageID +
                ', "' +
                result[list[key]].Path +
                '")'
            );
            item.style["background-image"] =
              "url(" + result[list[key]].Path + ")";
...
function submitBotCreation() {
  post("/createBot" /*"http://httpbin.org/post"*/, {
    nameID: nameID,
    imageID: picID,
    sex: sex
  });
